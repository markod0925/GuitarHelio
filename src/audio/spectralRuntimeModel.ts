...
import { resolveTargetGroupBounds } from '../guitar/targetGrouping';
import type { TargetNote } from '../types/models';
import type { SpectralRuntimeChord, SpectralRuntimeModel, SpectralRuntimeNote } from './pitchDetector';

const DEFAULT_MAX_FRET = 12;

export function buildSpectralRuntimeModelFromTargets(targets: TargetNote[]): SpectralRuntimeModel | null {
  if (targets.length === 0) return null;

  const noteByKey = new Map<string, SpectralRuntimeNote>();
  ...
  const notes = [...noteByKey.values()].sort(
    (a, b) => a.midi - b.midi || a.string - b.string || a.fret - b.fret || a.id.localeCompare(b.id)
  );
...
export function buildPracticeSpectralRuntimeModel(maxFret: number = DEFAULT_MAX_FRET): SpectralRuntimeModel {
  const notes: SpectralRuntimeNote[] = [];
...
function dedupePatternChords(
  targets: TargetNote[],
  noteIdsByMidi: Map<number, string[]>
): SpectralRuntimeChord[] {
  const chords: SpectralRuntimeChord[] = [];
...
function noteKey(string: number, fret: number, midi: number): string {
  return `${string}:${fret}:${midi}`;
}

function uniqueSortedMidi(values: number[]): number[] {
