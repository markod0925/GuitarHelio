...
export function buildGameplayValidationDebugSnapshot(
  ...
  const snapshot: GameplayValidationDebugSnapshot = {
    ...
    detector: {
      ...
      nativePayloadEnergyRatioDb: latestFrame?.native_debug?.energy_ratio_db ?? null,
      onsetStrength: latestFrame?.onset_strength ?? null,
      nativePayloadOnsetStrength: latestFrame?.native_debug?.onset_strength ?? null,
      contaminationScore: latestFrame?.contamination_score ?? null,
      nativePayloadContaminationScore: latestFrame?.native_debug?.contamination_score ?? null,
