                                         ...
                                         bool inNewAudio)
{
    ...
    const auto raw_max_note_idx =
        inParams.maxFrequency < 0 ? n_notes - 1 : NoteUtils::hzToMidi(inParams.maxFrequency) - MIDI_OFFSET;
    const auto raw_min_note_idx = inParams.minFrequency < 0 ? 0 : NoteUtils::hzToMidi(inParams.minFrequency) - MIDI_OFFSET;

    // Clamp note indexes before using them as vector indexes.
    // Without clamping, low minFrequency values (for example 1 Hz) produce negative note indexes.
    const auto max_note_idx = std::max(0, std::min(n_notes - 1, raw_max_note_idx));
    const auto min_note_idx = std::max(0, std::min(n_notes - 1, raw_min_note_idx));
    if (max_note_idx < min_note_idx) {
        return events;
    }
