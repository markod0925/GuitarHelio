...
type AndroidSeed = {
  row: Awaited<ReturnType<typeof buildDatasetRows>>[number];
  midi: number;
};
...
async function loadAndroidSeeds(repoRoot: string): Promise<AndroidSeed[]> {
...
async function loadGuitarSetSeeds(repoRoot: string): Promise<GuitarSetSeed[]> {
...
async function runDatasetSweep(input: {
  ...
  for (let index = 0; index < input.seeds.length; index += 1) {
    ...
    const entry = await buildDatasetEntry(seed, input.dataset, input.windowConfig);
  ...
  const ranked = rankVariantSummaries(variantSummaries);
...
async function buildDatasetEntry(
  ...
  dataset: DatasetName,
  windowConfig: AndroidWindowConfig | GuitarSetWindowConfig
): Promise<DatasetFileEntry> {
  if (dataset === 'android') {
    const androidSeed = seed as AndroidSeed;
    const durationSec = androidSeed.row.durationSec ?? 0;
    return {
  ...
  const guitarSeed = seed as GuitarSetSeed;
  const durationSec = await resolveAudioDuration(guitarSeed.wavPath);
  return {
...
async function resolveAudioDuration(filePath: string): Promise<number> {
  const decoded = await decodeMonoAudio(filePath);
...
function finalizeVariantAggregate(accumulator: VariantAccumulator, traceSummary: TraceSummary): MonoAggregateMetrics {
  const positiveConfirmed = accumulator.positiveConfirmedWindows;
  const recall = accumulator.positiveWindows > 0 ? positiveConfirmed / accumulator.positiveWindows : 0;
  const precision = accumulator.confirmedWindows > 0 ? positiveConfirmed / accumulator.confirmedWindows : 0;
  const lowBandTar = accumulator.lowBandPositiveWindows > 0 ? accumulator.lowBandConfirmedWindows / accumulator.lowBandPositiveWindows : null;
  const lowBandFar = accumulator.lowBandNegativeWindows > 0 ? accumulator.lowBandConfirmedWindows / accumulator.lowBandNegativeWindows : null;
...
async function runDetailedVariant(input: {
  ...
  windowConfig: AndroidWindowConfig | GuitarSetWindowConfig;
  config: MonoBenchmarkConfig;
}): Promise<DatasetSweepRun['detailed']> {
...
function buildDatasetEntry(
  ...
  const guitarSeed = seed as GuitarSetSeed;
  return resolveAudioDuration(guitarSeed.wavPath).then((durationSec) => ({
    fileId: guitarSeed.fileId,
    filePath: guitarSeed.wavPath,
    relativeFilePath: guitarSeed.wavRelativePath,
    durationSec,
...
function combineVariantRows(android: VariantSummary[], guitarset: VariantSummary[]): CombinedRow[] {
  ...
  for (const row of android) {
    const match = guitarByKey.get(row.key);
    if (!match) continue;
    rows.push({
      key: row.key,
      config: row.config,
      android: row.aggregate,
      guitarset: match.aggregate
    });
  }
  return rows;
}
...
function rankVariantSummaries(summaries: VariantSummary[]): VariantSummary[] {
  return [...summaries].sort((left, right) => compareVariantSummaries(left, right));
}

function compareVariantSummaries(left: VariantSummary, right: VariantSummary): number {
...
async function writeOutputs(input: {
  ...
  const leaderboard = buildLeaderboardDoc(input.android, input.guitarset, input.combinedRows, input.bestCombined);
  ...
  await fs.writeFile(path.join(input.outputDir, 'sweep', 'leaderboard.json'), `${JSON.stringify(leaderboard, null, 2)}\n`, 'utf8');
...
function buildDetailedDoc(dataset: DatasetName, datasetRoot: string, generatedAtIso: string, run: DatasetSweepRun): Record<string, unknown> {
  return {
    generatedAtIso,
    dataset,
    datasetRoot,
...
function buildDatasetSummaryMarkdown(title: string, run: DatasetSweepRun): string {
  ...
  if (run.dataset === 'android') {
    lines.push(
      '## Low String View',
      '',
      `- Low-string TAR: ${formatNullable(aggregate.lowBandTar, 3)}`,
      `- Low-string FAR: ${formatNullable(aggregate.lowBandFar, 3)}`,
      ''
    );
  } else {
    lines.push(
  ...
  lines.push(
    '## Window Timing',
    '',
    `- Average support seconds: ${formatNumber(aggregate.averageSupportSeconds, 4)}`,
    `- Average target-hit ratio: ${formatPct(aggregate.averageTargetHitRatio)}`,
    `- Average wrong-note ratio: ${formatPct(aggregate.averageWrongNoteRatio)}`,
    `- Confirmation latency avg/p95 (ms): ${formatNullable(aggregate.averageConfirmationLatencyMsAvg, 3)} / ${formatNullable(aggregate.averageConfirmationLatencyMsP95, 3)}`,
    `- Confirmation latency from onset avg/p95 (ms): ${formatNullable(aggregate.averageConfirmationLatencyFromTargetOnsetMsAvg, 3)} / ${formatNullable(aggregate.averageConfirmationLatencyFromTargetOnsetMsP95, 3)}`,
    `- First target-hit latency avg/p95 (ms): ${formatNullable(aggregate.firstTargetHitLatencyMsAvg, 3)} / ${formatNullable(aggregate.firstTargetHitLatencyMsP95, 3)}`,
    `- First any-hit latency avg/p95 (ms): ${formatNullable(aggregate.firstAnyHitLatencyMsAvg, 3)} / ${formatNullable(aggregate.firstAnyHitLatencyMsP95, 3)}`,
    `- No-detect frame ratio: ${formatPct(aggregate.noDetectFrameRatio)}`,
    `- Reset count avg: ${formatNumber(aggregate.resetCountAvg, 4)}`,
    ''
  );
  return lines.join('\n');
}
...
function buildRuntimeMarkdown(android: DatasetSweepRun, guitarset: DatasetSweepRun): string {
  return [
    ...
    '- End-to-end runtime is measured per window and includes the detector slice plus validator overhead.',
    '',
    '## Android',
    '',
...
function buildLeaderboardDoc(
  ...
  return {
    ...
    rankingPolicy: [
      'Android TAR descending',
      'Android note-mismatch FAR ascending',
      'Android strict FAR ascending',
      'Android low-string FAR ascending',
      'GuitarSet recall descending',
      'GuitarSet note-mismatch FAR ascending',
      'GuitarSet strict FAR ascending',
      'Total runtime ascending',
      'Required consecutive frames ascending',
      'FFT size ascending',
...
function buildLeaderboardMarkdown(
  ...
  rankVariantSummaries(android.variantSummaries).slice(0, 10).forEach((row, index) => {
    lines.push(
      `| ${index + 1} | ${formatBenchmarkVariantLabel(row.config)} | ${formatPct(row.aggregate.tar)} | ${formatPct(row.aggregate.strictFar)} | ${formatPct(row.aggregate.noteMismatchFar)} | ${formatNullable(row.aggregate.lowBandFar, 3)} | ${formatNumber(row.aggregate.totalRuntimeAvgMs, 4)} |`
    );
  });
  ...
  rankVariantSummaries(guitarset.variantSummaries).slice(0, 10).forEach((row, index) => {
    lines.push(
      `| ${index + 1} | ${formatBenchmarkVariantLabel(row.config)} | ${formatPct(row.aggregate.recall)} | ${formatPct(row.aggregate.strictFar)} | ${formatPct(row.aggregate.noteMismatchFar)} | ${formatNumber(row.aggregate.totalRuntimeAvgMs, 4)} |`
    );
  });
  ...
  lines.push(
    ...
    '## Recommended Variant',
    '',
    bestCombined ? `- ${formatBenchmarkVariantLabel(bestCombined.config)}` : '- none',
    ''
  );
  return lines.join('\n');
}
...
function buildLeaderboardDoc(android: DatasetSweepRun, guitarset: DatasetSweepRun, combinedRows: CombinedRow[], bestCombined: CombinedRow | null): Record<string, unknown> {
  return {
    generatedAtIso: new Date().toISOString(),
    rankingPolicy: [
      'Android TAR descending',
...
function isGoodFrame(observation: { detectorAccepted: boolean; detectedMidi: number | null; detectedHz: number | null }, expectedMidi: number | null, pitchToleranceCents: number): boolean {
  if (expectedMidi === null || observation.detectedMidi === null || observation.detectedHz === null || !observation.detectorAccepted) return false;
  const expectedHz = midiForStringFret(1, 0); // placeholder to keep deterministic type flow
  void expectedHz;
  return Math.abs(centsDifference(observation.detectedHz, expectedMidiToHz(expectedMidi))) <= pitchToleranceCents;
}

function expectedMidiToHz(midi: number): number {
  return 440 * 2 ** ((midi - 69) / 12);
}

function formatCsvValueWrapper(value: unknown): string {
...
function buildCsv(results: MonoWindowResult[]): string {
  ...
  for (const result of results) {
    lines.push([
      ...
      result.validatorRuntimeMs,
      result.totalRuntimeMs
    ].map(formatCsvValueWrapper).join(','));
  }
  return lines.join('\n');
}
...
function makeTraceSummary(config: MonoBenchmarkConfig, trace: StreamingTrace): TraceSummary {
  return {
    sampleRateMode: config.sampleRateMode,
    fftSize: config.fftSize,
    detectorRuntimeAvgMs: average(trace.observations.map((observation) => observation.runtimeMs)),
    detectorRuntimeP95Ms: percentile(trace.observations.map((observation) => observation.runtimeMs), 0.95)
  };
}

function buildTraceRuntimeSummary(accumulator: TraceAccumulator, config: MonoBenchmarkConfig): TraceSummary {
  return {
    sampleRateMode: config.sampleRateMode,
    fftSize: config.fftSize,
    detectorRuntimeAvgMs: average(accumulator.runtimeSamples),
