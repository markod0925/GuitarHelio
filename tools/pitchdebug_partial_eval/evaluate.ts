...
type PipelineSpec = {
  id: 'raw' | 'hpf50_lpf2000';
  label: string;
  createProcessor: (frameSize: number) => ((rawFrame: Float32Array, sampleRate: number) => Float32Array);
};

type FileSignalSummary = {
  medianRmsDbfs: number | null;
  medianSnrDb: number | null;
  medianLowBandEnergyRatio: number | null;
};

type FileAlgorithmResult = {
  algorithm: string;
  predictedFrequencyHz: number | null;
  ...
  accurate50Cents: boolean;
  octaveError: boolean;
  stringCorrect: boolean | null;
  fretCorrect: boolean | null;
  stringFretCorrect: boolean | null;
};
...
type FileEvaluation = {
  ...
  signal: FileSignalSummary;
  algorithms: FileAlgorithmResult[];
};
...
type AggregateMetrics = {
  ...
  stringFretAccuracy: number | null;
  octaveErrorRate: number;
  medianAcceptedFrameRate: number | null;
};

type AggregateGroup = Record<string, AggregateMetrics>;

type PipelineEvaluation = {
  fileEvaluations: FileEvaluation[];
  aggregatesByAlgorithm: Record<string, AggregateMetrics>;
  aggregatesByAlgorithmAndString: Record<string, AggregateGroup>;
  aggregatesByAlgorithmAndFret: Record<string, AggregateGroup>;
};

type ResultsDocument = {
  generatedAtIso: string;
  datasetPath: string;
  datasetPathWindows: string;
  outputDir: string;
  optionalPipelinesNotRun: string[];
  mapping: {
    standardTuningMidi: Record<number, number>;
    formula: string;
    openStringNotes: Array<{ stringId: number; midi: number; note: string; frequencyHz: number }>;
  };
  dataset: {
    fileCount: number;
    stringsCovered: number[];
    fretsCovered: number[];
    files: DatasetRow[];
  };
  frameConfig: {
    ...
    hopSize: number;
    fftSize: number;
    sampledWindowsPerFile: number;
    windowMarginSec: number;
  };
  algorithms: string[];
  pipelines: Record<string, PipelineEvaluation>;
};
...
const ALGORITHMS = ['ac14', 'spectral_game_runtime_unified_v3', 'MASP', 'FRETNET'] as const;
...
async function main(): Promise<void> {
  ...
  try {
    const pipelineResults: Record<string, PipelineEvaluation> = {};
...
async function evaluatePipeline(
  ...
  spectralModel: ReturnType<typeof buildPracticeSpectralRuntimeModel>
): Promise<PipelineEvaluation> {
  const fileEvaluations: FileEvaluation[] = [];
  for (let rowIndex = 0; rowIndex < datasetRows.length; rowIndex += 1) {
    ...
    const signalRmsDbfs: number[] = [];
    const signalSnrDb: number[] = [];
    const signalLowBandRatio: number[] = [];
    const frameResults = new Map<string, PitchDetectorResult[]>();
    for (const detector of detectors) {
    ...
    const algorithms = detectors.map((detector) =>
      summarizeAlgorithmResult(
        detector.name,
        frameResults.get(detector.name) ?? [],
        row.stringId,
        row.fret,
...
function summarizeAlgorithmResult(
  algorithm: string,
...
function buildAggregatesByAlgorithm(fileEvaluations: FileEvaluation[]): Record<string, AggregateMetrics> {
  const result: Record<string, AggregateMetrics> = {};
  for (const algorithm of ALGORITHMS) {
    const rows = fileEvaluations.map((file) => file.algorithms.find((entry) => entry.algorithm === algorithm)).filter((value): value is FileAlgorithmResult => Boolean(value));
    result[algorithm] = computeAggregateMetrics(rows);
  }
  return result;
}

function buildGroupedAggregates(
  fileEvaluations: FileEvaluation[],
  groupKey: (evaluation: FileEvaluation) => string
): Record<string, AggregateGroup> {
  const grouped: Record<string, AggregateGroup> = {};
  for (const algorithm of ALGORITHMS) {
    const groups = new Map<string, FileAlgorithmResult[]>();
...
function computeAggregateMetrics(rows: FileAlgorithmResult[]): AggregateMetrics {
  const fileCount = rows.length;
  const detected = rows.filter((row) => !row.noDetection);
  const cents = detected.map((row) => row.centsError).filter((value): value is number => value !== null);
  const absCents = detected.map((row) => row.absCentsError).filter((value): value is number => value !== null);
...
async function writeResults(outputDir: string, results: ResultsDocument): Promise<void> {
  await fs.writeFile(path.join(outputDir, 'results.json'), `${JSON.stringify(results, null, 2)}\n`, 'utf8');
  await fs.writeFile(path.join(outputDir, 'results.csv'), `${buildCsv(results)}\n`, 'utf8');
}

async function writeSummary(outputDir: string, results: ResultsDocument): Promise<void> {
  const primary = results.pipelines[PRIMARY_PIPELINE];
  const rankedByAccuracy = [...ALGORITHMS].sort((left, right) =>
  ...
  const rankedByMedian = [...ALGORITHMS].sort((left, right) =>
    compareNullable(primary.aggregatesByAlgorithm[left].medianAbsCentsError, primary.aggregatesByAlgorithm[right].medianAbsCentsError) ||
    primary.aggregatesByAlgorithm[right].pitchAccuracy50Cents - primary.aggregatesByAlgorithm[left].pitchAccuracy50Cents
  );
  const rankedByFailures = [...ALGORITHMS].sort((left, right) =>
    primary.aggregatesByAlgorithm[left].noDetectionRate - primary.aggregatesByAlgorithm[right].noDetectionRate ||
    primary.aggregatesByAlgorithm[right].pitchAccuracy50Cents - primary.aggregatesByAlgorithm[left].pitchAccuracy50Cents
  );
  ...
  const lowStringAccuracy = Object.fromEntries(ALGORITHMS.map((algorithm) => [
    algorithm,
    computeAggregateMetrics(lowStringRows.map((row) => row.algorithms.find((entry) => entry.algorithm === algorithm)).filter((value): value is FileAlgorithmResult => Boolean(value))).pitchAccuracy50Cents
  ]));
  const octaveCounts = Object.fromEntries(ALGORITHMS.map((algorithm) => [
    ...
    primary.fileEvaluations
      ...
      .filter((value): value is FileAlgorithmResult => Boolean(value))
...
function inferIssueAttribution(results: ResultsDocument): string {
  ...
  const medianSignalDbfs = median(results.pipelines.raw.fileEvaluations.map((row) => row.signal.medianRmsDbfs).filter((value): value is number => value !== null));
...
async function writePlots(plotsDir: string, results: ResultsDocument): Promise<void> {
  const primary = results.pipelines[PRIMARY_PIPELINE];
  for (const algorithm of ALGORITHMS) {
...
function buildCsv(results: ResultsDocument): string {
  const header = [
    'pipeline',
    'file_path',
    'string',
...
function buildScatterSvg(fileEvaluations: FileEvaluation[], algorithm: string): string {
  const width = 800;
  const height = 560;
...
function buildHistogramSvg(fileEvaluations: FileEvaluation[]): string {
  const width = 920;
  const height = 560;
  const margin = { left: 72, right: 24, top: 52, bottom: 64 };
  const innerWidth = width - margin.left - margin.right;
  const innerHeight = height - margin.top - margin.bottom;
  const allErrors = ALGORITHMS.flatMap((algorithm) =>
    fileEvaluations
  ...
  const binsByAlgorithm = new Map<string, number[]>();
  let maxBin = 1;
  for (const algorithm of ALGORITHMS) {
...
function buildPerStringSvg(pipeline: PipelineEvaluation): string {
  const width = 920;
  const height = 560;
  const margin = { left: 72, right: 24, top: 52, bottom: 64 };
  const innerWidth = width - margin.left - margin.right;
  ...
  for (const stringId of strings) {
    ...
    for (const algorithm of ALGORITHMS) {
      ...
      if (subX === null) continue;
      const metrics = computeAggregateMetrics(groupRows.map((row) => row.algorithms.find((entry) => entry.algorithm === algorithm)).filter((value): value is FileAlgorithmResult => Boolean(value)));
...
function buildPerFretHeatmapSvg(pipeline: PipelineEvaluation): string {
...
function buildFailureSvg(fileEvaluations: FileEvaluation[]): string {
  const width = 920;
  const height = 420;
  const margin = { left: 72, right: 24, top: 52, bottom: 64 };
  const innerWidth = width - margin.left - margin.right;
  ...
  for (let index = 0; index < ALGORITHMS.length; index += 1) {
    ...
    for (const evaluation of fileEvaluations) {
      ...
      if (!result?.noDetection) continue;
...
function buildAxes(
  ...
  xTicks: Array<{ value: number; label?: string }> | Array<number>,
  yTicks: Array<{ value: number; label?: string }> | Array<number>,
  xProject: (value: number) => number,
  yProject: (value: number) => number,
  xLabel: string,
  yLabel: string
): string {
  const elements: string[] = [];
...
function buildSampledFrameStarts(sampleCount: number, sampleRate: number): number[] {
...
function buildPercentTicks(): number[] {
...
function bandScale(values: string[], rangeMin: number, rangeMax: number, paddingInner: number) {
  ...
  return {
    ...
    centerForValue: (value: number) => {
      const position = positions.get(String(value)) ?? positions.get(value as unknown as string) ?? null;
