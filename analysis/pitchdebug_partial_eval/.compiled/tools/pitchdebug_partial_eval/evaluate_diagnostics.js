...
import { buildFftPlan, computeMagnitudeSpectrum, computePeak, computeRms, computeTopSpectralPeaks, fillWindowKernel } from '../../src/audio/debugSignalProcessing.js';
...
import { midiForStringFret } from '../../src/guitar/tuning.js';
...
const ALGORITHMS = ['ac14', 'spectral_game_runtime_unified_v3', 'MASP', 'FRETNET', 'pyin'];
const EXPECTED_STRINGS = [1, 2, 3, 4, 5, 6];
const EXPECTED_TAKES = [1, 2, 3];
...
const TIME_WINDOWS = [
    ...
    { id: 'center_window', label: 'Center window (450 ms)', kind: 'center', durationSec: 0.45, startOffsetSec: 0, targetFrames: 8 },
    { id: 'sustain_window', label: 'Sustain window (750 ms)', kind: 'sustain', durationSec: 0.75, startOffsetSec: 0, targetFrames: 12 },
    { id: 'onset_skipped_window', label: 'Onset skipped (200 ms + 550 ms)', kind: 'offset', durationSec: 0.55, startOffsetSec: 0.20, targetFrames: 9 },
    { id: 'sustain_long_window', label: 'Sustain long (1200 ms)', kind: 'sustain', durationSec: 1.2, startOffsetSec: 0, targetFrames: 18 }
];
const FRONTENDS = [
    { id: 'raw', label: 'RAW' }
];
...
async function main() {
    ...
    const pyinTraces = new Map();
    const decodeFailures = new Map();
    try {
        for (let index = 0; index < datasetRows.length; index += 1) {
            ...
            try {
                ...
                decodeFailures.set(row.relativeFilePath, error instanceof Error ? (error.message || 'decode_failed') : String(error));
                continue;
            }
            ...
            const rawWindowFeatures = new Map();
            ...
            for (const timeWindow of TIME_WINDOWS) {
                ...
                rawWindowFeatures.set(timeWindow.id, computeSegmentFeature(timeWindow.id, rawSegment, decoded.sampleRate, groundTruthFrequencyHz, bounds.startSample / decoded.sampleRate));
            }
            for (const timeWindow of TIME_WINDOWS) {
                const rawSegment = rawSegments.get(timeWindow.id);
                const windowFeature = rawWindowFeatures.get(timeWindow.id);
                if (!rawSegment || !windowFeature)
                    continue;
                ...
                const perDetectorResults = new Map();
                for (const detector of detectors) {
                ...
                for (const detector of detectors) {
                    diagnosticRows.push(summarizeDiagnosticRow({
                        row,
                ...
                const pyinFrameResults = selectPyinFrameResultsForWindow(pyinTrace, rawSegment.startSample / decoded.sampleRate, rawSegment.endSample / decoded.sampleRate, timeWindow.targetFrames);
                diagnosticRows.push(summarizeDiagnosticRow({
                    row,
                    algorithm: 'pyin',
                    frontend: 'raw',
...
async function resolvePyinCliPath(repoRoot) {
    const candidates = [
...
async function runPyinTraceForFile(input) {
    ...
    const run = (streamingRun.run ?? {});
    const rawFrames = Array.isArray(run.frames) ? run.frames : [];
    const pyinConfig = (runtimeConfig.pyin ?? {});
    ...
    const inferredFrameLength = finiteInteger(pyinConfig.frame_length) ?? deriveDefaultPyinFrameLength(runtimeSampleRate, runtimeBlockSize, fminHz);
    ...
    return {
        ...
        runtimeBlockSize,
        runtimeCallbackSize: finiteInteger(run.summary?.callback_size) ?? PYIN_CALLBACK_SIZE,
        runtimeHopSize,
        fminHz,
        fmaxHz,
        inferredFrameLength,
...
function deriveDefaultPyinFrameLength(sampleRate, runtimeBlockSize, fminHz) {
    const safeRuntimeBlock = Math.max(256, runtimeBlockSize);
    if (!(fminHz > 0))
        return safeRuntimeBlock;
...
function selectPyinFrameResultsForWindow(trace, windowStartSec, windowEndSec, targetFrames) {
...
async function buildDatasetIntegrityReport(datasetDir) {
    const audioDir = path.join(datasetDir, 'audio');
    const entries = await fs.readdir(audioDir);
...
function buildTakeFeatures(row, samples, sampleRate, groundTruthMidi, groundTruthFrequencyHz) {
...
function computeSegmentFeature(segmentId, samples, sampleRate, expectedFrequencyHz, startSec) {
...
function buildAverageSpectrum(samples, sampleRate) {
...
function buildSpectrumFrameStarts(sampleCount, frameSize, hopSize) {
...
function resolveTimeWindowBounds(sampleCount, sampleRate, spec) {
...
function toSampleBounds(startSec, durationSec, sampleRate, sampleCount) {
...
function buildVariantFrameStarts(sampleCount, sampleRate, targetFrames) {
...
function applyFrontend(source, sampleRate, frontendId) {
...
function summarizeDiagnosticRow(input) {
    ...
    const predictedFrequencyHz = median(accepted.map((result) => finiteNumber(result.pitchHz)).filter((value) => value !== null));
    ...
    const dominantDebugResampleMode = mostCommon(input.frameResults.map((result) => {
        const mode = result.debug?.resampleMode;
...
function classifyError(predictionHz, centsError, absCentsError) {
    if (predictionHz === null)
        return 'no_detection';
    if (!Number.isFinite(predictionHz) || predictionHz <= 0)
        return 'invalid_output';
    if (absCentsError !== null && absCentsError <= 50)
        return 'correct_within_50c';
    if (centsError !== null && Math.abs(centsError - 1200) <= 120)
        return 'octave_up_error';
    if (centsError !== null && Math.abs(centsError + 1200) <= 120)
        return 'octave_down_error';
    if (centsError !== null) {
    ...
    if (absCentsError !== null && absCentsError <= 150)
        return 'near_miss_non_octave';
    return 'large_error';
}
function computeAggregate(rows) {
    ...
    return {
        ...
        medianAbsCentsError: roundNullable(median(rows.map((row) => row.absCentsError).filter((value) => value !== null)), 3),
        medianSignedCentsError: roundNullable(median(rows.map((row) => row.centsError).filter((value) => value !== null)), 3),
...
async function writeFeaturesOutputs(outputDir, featuresRows) {
    const jsonDoc = {
...
async function writeResultsOutputs(outputDir, datasetRows, diagnosticRows, datasetIntegrity, maspAudit, pyinAudit, takeConsistencyRows, lowMidHighByAlgorithm, rankings) {
    const rawFull = selectRows(diagnosticRows, 'raw', 'full_take');
    const aggregatesByAlgorithm = Object.fromEntries(ALGORITHMS.map((algorithm) => [algorithm, computeAggregate(rawFull.filter((row) => row.algorithm === algorithm))]));
    const aggregatesByAlgorithmAndTimeWindow = Object.fromEntries(TIME_WINDOWS.map((window) => [
        window.id,
        Object.fromEntries(ALGORITHMS.map((algorithm) => [
            algorithm,
            computeAggregate(selectRows(diagnosticRows, 'raw', window.id).filter((row) => row.algorithm === algorithm))
        ]))
    ]));
    const errorTypeDistribution = Object.fromEntries(ALGORITHMS.map((algorithm) => [
        algorithm,
...
function buildMaspAudit(diagnosticRows) {
...
function buildPyinAudit(diagnosticRows, pyinTraces, cliPath) {
    const rawFull = selectRows(diagnosticRows, 'raw', 'full_take');
...
function buildTakeConsistencyRows(diagnosticRows) {
    const rawFull = selectRows(diagnosticRows, 'raw', 'full_take');
    const out = [];
    for (const algorithm of ALGORITHMS) {
        ...
        for (const [key, rows] of grouped.entries()) {
            ...
            const cents = sorted.map((row) => row.centsError).filter((value) => value !== null);
            ...
            const consistencyRate = sorted.length > 0
                ...
                : 0;
            const predictedMidis = sorted.map((row) => row.predictionMidi).filter((value) => value !== null);
            ...
            const unstableScore = (std ?? 180) / 100 +
                (1 - consistencyRate) +
    ...
    return out.sort((left, right) => left.algorithm.localeCompare(right.algorithm) ||
        right.stringId - left.stringId ||
        left.fret - right.fret);
}
function buildLowMidHighMetrics(diagnosticRows) {
    const rawFull = selectRows(diagnosticRows, 'raw', 'full_take');
    const groups = [
    ...
    for (const algorithm of ALGORITHMS) {
        ...
        for (const group of groups) {
            out[algorithm][group.id] = computeAggregate(rawFull.filter((row) => row.algorithm === algorithm && group.includes(row.stringId)));
        }
    }
    return out;
}
function buildAlgorithmRankings(diagnosticRows, takeConsistencyRows, lowMidHighByAlgorithm) {
    const rawFull = selectRows(diagnosticRows, 'raw', 'full_take');
    ...
    const overall = [...entries].sort((left, right) => left.compositeRankScore - right.compositeRankScore ||
        right.overallAccuracy - left.overallAccuracy);
    const lowFrequency = [...entries].sort((left, right) => right.lowStringAccuracy - left.lowStringAccuracy ||
...
function rankingMap(entries, valueOf, descending) {
...
function pairwiseAgreementRate(values) {
    if (values.length < 2)
        return null;
    let pairs = 0;
    let equalPairs = 0;
...
async function writePlots(plotsDir, diagnosticRows, featuresRows) {
    const rawFull = selectRows(diagnosticRows, 'raw', 'full_take');
...
async function writeSummary(outputDir, datasetRows, diagnosticRows, datasetIntegrity, maspAudit, pyinAudit, takeConsistencyRows, lowMidHighByAlgorithm, rankings) {
    const rawFull = selectRows(diagnosticRows, 'raw', 'full_take');
...
function chooseRepresentativeCases(rawFull, featuresRows) {
...
function buildRepresentativeCaseSvg(caseInfo) {
    const width = 860;
    const height = 420;
...
function buildHpfAuditSvg(rawFrame, legacyFrame, continuousFrame, sampleRate, fileId) {
    const width = 960;
    const height = 560;
    const mid = 280;
...
function buildWaveformPanel(samples, x, y, width, height, color, label) {
...
function buildBinaryScatterSvg(rows, xField, title, xLabel) {
    const width = 880;
    const height = 460;
...
function buildMetricByAlgorithmSvg(rawFull, title, metricOf, options) {
    const width = 920;
    const height = 460;
...
function buildGroupedAccuracySvg(diagnosticRows, fixedValue, varyingField, ids, labels, title) {
    const width = 980;
    const height = 520;
...
function buildErrorTypeDistributionSvg(rawFull) {
    const width = 980;
    const height = 520;
...
function buildFailureRateVsFrequencySvg(rawFull) {
    const grouped = new Map();
    for (const algorithm of ALGORITHMS) {
...
function buildFeaturesCsv(rows) {
    ...
    for (const row of rows) {
        ...
        for (const segmentId of segments) {
            ...
            values.push(segment.startSec, segment.endSec, segment.durationSec, segment.rms, segment.peakAbs, segment.crestFactor, segment.mean, segment.dcOffset, segment.energy_20_50_hz, segment.energy_50_90_hz, segment.energy_80_120_hz, segment.energy_120_250_hz, segment.energy_250_500_hz, segment.energy_500_1000_hz, segment.energy_1000_2000_hz, segment.low_to_mid_ratio, segment.spectral_tilt_80_120_vs_500_1000_db, segment.fund_energy, segment.harmonic2_energy, segment.harmonic3_energy, segment.fund_to_2harm_ratio, segment.fund_to_2harm_ratio_db, segment.fund_to_3harm_ratio, segment.fund_to_3harm_ratio_db, segment.second_harmonic_stronger_than_fundamental, segment.third_harmonic_stronger_than_fundamental, segment.topPeakHz, segment.topPeak2Hz);
        }
        csvRows.push(values.map((value) => csvEscape(formatCsvValue(value))).join(','));
    }
    return csvRows.join('\n');
}
function buildDiagnosticCsv(rows) {
...
function selectRows(rows, frontend, timeWindowVariant) {
...
function bandEnergy(spectrum, sampleRate, fftSize, minHz, maxHz) {
...
function removeMeanInPlace(samples) {
    const mean = average(Array.from(samples));
...
function designHighPassBiquad(sampleRate, cutoffHz, q) {
...
function designLowShelfBiquad(sampleRate, cutoffHz, slope, gainDb) {
...
function applyLegacyContinuousHpfLpfInPlace(samples, sampleRate, hpfHz, lpfHz) {
    if (samples.length <= 1)
        return;
...
function onePoleHighPassGain(sampleRate, cutoffHz, frequencyHz) {
...
function onePoleLowPassGain(sampleRate, cutoffHz, frequencyHz) {
...
function combinedOnePoleGainDb(sampleRate, highPassHz, lowPassHz, frequencyHz) {
...
function formatNullable(value, digits, unit) {
...
function markerPosition(frequencyHz, minHz, maxHz, rangeMin, rangeMax) {
