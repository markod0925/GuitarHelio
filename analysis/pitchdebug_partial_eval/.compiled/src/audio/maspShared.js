...
export function resolveMaspResampleMode(sampleRate) {
    if (!Number.isFinite(sampleRate) || sampleRate <= 0)
        return 'unsupported';
    if (Math.abs(sampleRate - 22050) <= 1)
        return 'native_22050';
    if (Math.abs(sampleRate - 44100) <= 1)
        return 'decimate_44100';
    if (Math.abs(sampleRate - 48000) <= 1)
        return 'linear_48000';
    return 'unsupported';
}
export function sanitizeMaspValidationContext(context) {
    if (!context)
        return null;
...
export function buildMaspValidationContextForTargetGroup(targetGroup, tickToSeconds, playheadSec, anticipationSec = MASP_PLAYHEAD_ANTICIPATION_SECONDS) {
    if (targetGroup.length === 0)
        return null;
    const sanitizedPlayhead = sanitizeFinite(playheadSec);
    if (sanitizedPlayhead === null)
        return null;
    const expectedNotes = [];
    ...
    for (const target of targetGroup) {
        ...
        if (midi === null || string === null || fret === null)
            continue;
        ...
        if (onsetSec === null || offsetSecRaw === null)
            continue;
