import { midiToHz } from '../ui/song-select/utils/songSelectUtils.js';
...
export function resolveCapturePresetLabel(mode) {
    if (mode === 'live_mic')
        return 'Live mic';
    if (mode === 'file')
        return 'File';
    if (mode === 'reference_test')
        return 'Reference test';
    return 'Replay';
}
export function fillWindowKernel(target, windowType) {
...
export function buildFftPlan(nfft) {
...
export function normalizeFrame(samples, targetPeak = 0.92) {
...
function onePoleFilterCoefficient(sampleRate, cutoffHz) {
...
export function applyHighPass(samples, sampleRate, cutoffHz) {
    if (samples.length <= 1)
        return;
...
export function applyLowPass(samples, sampleRate, cutoffHz) {
    if (samples.length <= 1)
        return;
...
export function applyNoiseGate(samples, gateThreshold) {
...
export function applyWindow(source, kernel, target) {
    const length = Math.min(source.length, kernel.length, target.length);
    for (let i = 0; i < length; i += 1) {
        target[i] = source[i] * kernel[i];
    }
}
export function computeMagnitudeSpectrum(samples, fftSize, plan, re, im, magnitude) {
    re.fill(0);
    im.fill(0);
...
export function computeAutocorrelationSummary(samples, sampleRate, minFrequencyHz = 65, maxFrequencyHz = 1200) {
...
export function computeTopSpectralPeaks(magnitudeSpectrum, sampleRate, fftSize, maxPeaks = 5) {
    ...
    for (let i = 1; i < magnitudeSpectrum.length - 1; i += 1) {
        ...
        if (!(value > magnitudeSpectrum[i - 1] && value >= magnitudeSpectrum[i + 1]))
            continue;
        peaks.push({
            bin: i,
...
function bandEnergy(magnitudeSpectrum, sampleRate, fftSize, minHz, maxHz) {
...
function energyNearFrequency(magnitudeSpectrum, sampleRate, fftSize, frequencyHz, bandwidthHz = 8) {
    return bandEnergy(magnitudeSpectrum, sampleRate, fftSize, Math.max(0, frequencyHz - bandwidthHz), frequencyHz + bandwidthHz);
}
export function computeSignalMetrics(frame, magnitudeSpectrum, sampleRate, fftSize, previousRms) {
...
export function buildSyntheticReferenceWav(selection, sampleRate = 48000, durationMs = 2200) {
...
export function buildDownloadFileName(prefix, extension) {
    const date = new Date();
    const stamp = [
        date.getFullYear(),
