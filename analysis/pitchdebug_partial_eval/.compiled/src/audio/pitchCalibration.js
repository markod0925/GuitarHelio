...
export function estimatePitchCalibrationMeasurement(midiSamples, targetMidi) {
    ...
    if (trimmed.length < MIN_MEASUREMENT_SAMPLES)
        return null;
...
export function buildPitchCalibrationProfile(measurements, createdAtMs = Date.now()) {
    const byMidi = new Map();
    for (const raw of measurements) {
        if (!raw || !Number.isFinite(raw.midi) || !Number.isFinite(raw.offsetCents))
            continue;
        ...
        const replace = point.sampleCount > existing.sampleCount ||
...
export function getPitchCalibrationOffsetCents(midiEstimate, profile) {
    if (!profile || !Array.isArray(profile.points) || profile.points.length === 0)
        return 0;
    if (!Number.isFinite(midiEstimate))
        return 0;
    const points = profile.points;
    if (points.length === 1)
        return points[0].offsetCents;
    if (midiEstimate <= points[0].midi)
        return points[0].offsetCents;
    if (midiEstimate >= points[points.length - 1].midi)
        return points[points.length - 1].offsetCents;
    for (let i = 0; i < points.length - 1; i += 1) {
        const left = points[i];
        const right = points[i + 1];
        if (midiEstimate < left.midi || midiEstimate > right.midi)
            continue;
        const span = right.midi - left.midi;
        if (span <= 0)
            return left.offsetCents;
        const t = (midiEstimate - left.midi) / span;
        return left.offsetCents + t * (right.offsetCents - left.offsetCents);
    }
    return points[points.length - 1].offsetCents;
}
export function applyPitchCalibration(midiEstimate, profile) {
    if (!Number.isFinite(midiEstimate))
        return midiEstimate;
    const correctionCents = getPitchCalibrationOffsetCents(midiEstimate, profile);
    return midiEstimate - correctionCents / 100;
}
export function loadPitchCalibrationProfile() {
...
export function savePitchCalibrationProfile(profile) {
    if (typeof window === 'undefined')
        return;
...
export function clearPitchCalibrationProfile() {
    if (typeof window === 'undefined')
        return;
    try {
...
function sanitizePitchCalibrationProfile(input) {
    ...
    for (const pointRaw of data.points) {
        ...
        if (!Number.isFinite(midiRaw) || !Number.isFinite(offsetRaw))
            continue;
        const point = {
    ...
    const deduped = Array.from(points.reduce((map, point) => {
